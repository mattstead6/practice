...
// does what happens inside the if statement affect the i 
// in the overall scope

console.log(stringSearch('he', 'h'))




const stringSearch2 = (long, short) => {
    for (let i = 0; i < long.length; i ++) {
       for (let j = 0; j < short.length; j++){
           if (short[j] !== long[i+j]){ // why do it this way?
              break;
           } 
       }
    }
...
function solution(numbers) {
    let possibleAnswers = [1,4,9,16,25,36,49,64,81,100,121,144,169]
    let count = 0;
    // let sortedArray = numbers.sort((a,b) => a-b)
    for (let i = 0; i < numbers.length; i ++){
        if (possibleAnswers.includes(i * i)) {
            count++
        }
        for (let j = i + 1; j < numbers.length; j++) {
             if (Number.isInteger(Math.sqrt(numbers[i] + numbers[j]))) {
            count++
        }
        }
